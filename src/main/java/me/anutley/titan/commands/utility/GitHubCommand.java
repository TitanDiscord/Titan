...
public class GitHubCommand extends Command {
    ...
    public void onSlashCommand(SlashCommandEvent event) {
        ...
        if (event.getSubcommandName().equals("organisation")) {
            try {
                JsonNode repo = requestGitHub("https://api.github.com/orgs/" + event.getOption("org").getAsString());

                if (repo.get("message") != null) {
                    event.replyEmbeds(new EmbedBuilder()
                            .setTitle("This organisation does not exist!")
                            .setColor(EmbedColour.NO.getColour())
                            .build()).queue();
                    return;
                }
                ...
                if (!repo.get("description").asText().equals("null")) builder.setDescription(repo.get("description").asText());
                if (mostPopularRepo != null)
                    builder.addField("Most Popular Repositories", "[" + mostPopularRepo.get("name").asText() + "](" + mostPopularRepo.get("html_url").asText() + ")", true);
    ...
    public JsonNode requestGitHub(String url) throws IOException {
    ...
    public String getCommandDescription() {
        return "Gets information about a GitHub repo";
    }

    @Override
    public String getCommandUsage() {
        return "/github <repository/user/organisation>";
    }
}
